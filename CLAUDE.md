...
- `Tower` → `BulletTower`, `LaserTower` (via `TowerFactory`)
- `Enemy` → `CircleEnemy`, `TriangleEnemy`, `SquareEnemy`, `HexagonEnemy` (via `EnemyFactory`)
...
- `tower-stats.ts` — Damage, fire rate, range, cost per tower type and level
- `enemy-configs.ts` — Health, speed, reward, color per enemy type
